...
export async function regenerateAiForecastAction() {
  ...
  if (currentStore?.store?.id) {
    ...
    await logActivity({
      ...
      userId: user?.id,
      action: "generate",
      entityType: "ai_forecast",
      title: "Forecast AI diperbarui",
      description: "Prediksi AI berhasil dibuat ulang menggunakan data terbaru.",
      metadata: {
  ...
  redirect("/prediksi-ai?toast=ai-forecast-generated");
}